%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%   CUSTIME stimulation script: Displays 30 sentences in the LfPC form   %%%%%%%%%%
%%%%%%%%       Sends triggers to an EEG and an Eyelink 1000 eye-tracking        s%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
% Stimuli parameters
nb_sentences_in_exp = 30; % Nb of sentences displayed during the entire experiment (to be splitted into several runs % Default = 30 (words to be displayed to the controls)
spacing_of_rests = 0; % Nb of sentence displays between rest periods % Default = 0
nb_runs = 1; % Nb of runs that will be runned in the subject's session, with the sentences to be distributed among them % Default = 1 (doesn't run with a different number)
...
file_name_et = fullfile(results_path_et, sprintf('sub-S%d_ses-%014s_task-SentencesCtrl_run-%01d.edf', subject_id, date_of_day, num_run));
...
file_custime_sentences_stim = fullfile(main_path, '/custime_stim_lists/custime_sentences_stim_ctrl.mat');
...
if ~isfile(file_custime_sentences_stim_subject) % Create a stim mat only if this hasn't be done yet for the considered subject
    ...
    load('custime_sentences_stim_ctrl');
    for i_trial=1:nb_sentences_in_exp
        structure_temp(i_trial).num_pair = custime_sentences_stim_ctrl(i_trial).num_pair;
        structure_temp(i_trial).sentence = custime_sentences_stim_ctrl(i_trial).sentence;
        structure_temp(i_trial).num_sentence = custime_sentences_stim_ctrl(i_trial).num_sent;
        structure_temp(i_trial).predictable = custime_sentences_stim_ctrl(i_trial).predictable;
        structure_temp(i_trial).diff_of_prob = custime_sentences_stim_ctrl(i_trial).diff_of_prob;
    end
    clear('custime_sentences_stim_ctrl');
    ...
    custime_sentences_stim_ctrl = [structure_temp, substructure];

    % Generate a list of the videos to be displayed
    list_videos = {};
    for i_trial=1:nb_sentences_in_exp
        list_videos(i_trial) = {fullfile(main_path,'custime_videos','custime_videos_sentences',sprintf('sent_%02d.mp4',custime_sentences_stim_ctrl(i_trial).num_sentence))};
    end
    ...
    for i_trial=1:nb_sentences_in_exp
        custime_sentences_stim_ctrl(i_trial).subject = subject_id;
        custime_sentences_stim_ctrl(i_trial).run = num_run;
        custime_sentences_stim_ctrl(i_trial).video = char(list_videos(i_trial));
        custime_sentences_stim_ctrl(i_trial).fixation_duration = fixation_duration_list(i_trial);
    end
    save(file_custime_sentences_stim_subject, 'custime_sentences_stim_ctrl');
end
...
log_structure(nb_sentences_in_exp) = struct('subject', [], 'run', [], ...
    'num_pair', [], 'sentence', [], 'num_sentence', [], 'predictable', [], 'diff_of_prob', [], ...
    'video', [], 'sentence_onset_time', [], 'sentence_offset_time', [], 'sentence_marker', [], ...
    'fixation_onset_time', [], 'fixation_duration', [], 'fixation_marker', []);
...
% Append the sentences and their information to the structure log
for i_trial=1:nb_sentences_in_exp
        log_structure(i_trial).subject = custime_sentences_stim_ctrl(i_trial).subject;
        log_structure(i_trial).run = custime_sentences_stim_ctrl(i_trial).run;
        log_structure(i_trial).num_pair = custime_sentences_stim_ctrl(i_trial).num_pair;
        log_structure(i_trial).sentence = custime_sentences_stim_ctrl(i_trial).sentence;
        log_structure(i_trial).num_sentence = custime_sentences_stim_ctrl(i_trial).num_sentence;
        log_structure(i_trial).predictable = custime_sentences_stim_ctrl(i_trial).predictable;
        log_structure(i_trial).diff_of_prob = custime_sentences_stim_ctrl(i_trial).diff_of_prob;
        log_structure(i_trial).video = custime_sentences_stim_ctrl(i_trial).video;
        log_structure(i_trial).fixation_duration = custime_sentences_stim_ctrl(i_trial).fixation_duration;
end
clear('custime_sentences_stim_ctrl');
...
if exp_term
    ...
    file_name_beh = fullfile(results_path_beh, sprintf('sub-S%d_ses-%014s_task-SentencesCtrl_run-%01d_beh.csv', subject_id, date_of_run, num_run));
...
task_instructions_text = 'Restez attentif aux videos';
