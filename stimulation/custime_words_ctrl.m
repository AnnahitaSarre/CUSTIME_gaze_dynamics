%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%  CUSTIME stimulation script: Displays 72 (pseudo)words in the LfPC form  %%%%%%%%
...
% Stimuli parameters
nb_words_in_exp = 72; % Nb of words displayed during the entire experiment (to be splitted into several runs) % Default = 72 (words to be displayed to the controls)
spacing_of_rests = 0; % Nb of word displays between rest periods % Default = 0
nb_runs = 1; % Nb of runs that will be runned in the subject's session, with the words to be distributed among them % Default = 1
...
file_name_et = fullfile(results_path_et, sprintf('sub-S%d_ses-%014s_task-WordsCtrl_run-%01d.edf', subject_id, date_of_day, num_run));
...
%% Generate the subject's list of stims, generate the order of trials and create the log structure

rng_struct = rng('shuffle'); % Initialize the random generator

file_custime_words_stim = fullfile(main_path, '/custime_stim_lists/custime_words_stim_ctrl.mat');
...
while 1
    ...
    custime_words_stim_ctrl = custime_words_stim_ctrl(randperm(nb_words_in_exp));
    for i_trial=2:nb_words_in_exp
        if custime_words_stim_ctrl(i_trial-1).pseudo && custime_words_stim_ctrl(i_trial).pseudo
            invalid_target = 1;
            break;
        end
    end
...
log_structure(nb_words_in_exp) = struct('subject', [], 'run', [], ...
    'word', [], 'num_in_cat', [], 'freq', [], 'mismatch', [], 'pseudo', [], ...
...
for i_trial=1:nb_words_in_exp
    log_structure(i_trial).subject = subject_id;
    log_structure(i_trial).run = num_run;
    log_structure(i_trial).word = custime_words_stim_ctrl(i_trial).word;
    log_structure(i_trial).num_in_cat = custime_words_stim_ctrl(i_trial).num_in_cat;
    log_structure(i_trial).freq = custime_words_stim_ctrl(i_trial).freq;
    log_structure(i_trial).mismatch = custime_words_stim_ctrl(i_trial).mismatch;
    log_structure(i_trial).pseudo = custime_words_stim_ctrl(i_trial).pseudo;
end

save(file_custime_words_stim_subject, 'custime_words_stim_ctrl');
clear('custime_words_stim_ctrl');
...
fixation_duration_list = linspace(fixation_duration_min, fixation_duration_max, nb_words_in_exp);
fixation_duration_list = fixation_duration_list(randperm(nb_words_in_exp));

% Determine the trials after which a rest period will happen
if spacing_of_rests > 0
    list_rests = [];
    rest_range = 1;
    while spacing_of_rests * rest_range < nb_words_in_exp
...
if exp_term
    ...
    file_name_beh = fullfile(results_path_beh, sprintf('sub-S%d_ses-%014s_task-WordsCtrl_run-%01d_beh.csv', subject_id, date_of_run, num_run));
